...
var path, includeStyle, fs, sane, buildUtils, buildOptions;
...
module.exports = {
	...
	configureFolders: function (options) {
		...
		options.outputFolder = `dist/${options.env}`;
		if (options.env != 'dev') {
			options.production = true;
		}
		options.autoBundle = false; /* autoBundle is to bundle all (routes,components,models,mixins,helpers and router.js file) into app.js */
		options.theming = false;
		options.useStrict = false;
		options.eslint = false;
		options.liveReload = false;
		options.toCopy = true; /* To copy all the files, other than mentioned in consolidated_files */
		buildUtils = require(path.join(options.cliRoot, 'lib', 'utilities', 'buildUtils'));
		var folders = {
			/* Available modules and its folder structures.*/
			build: 'build',
			routes: 'routes',
			routers: 'routes/router.js',
			...
			styles: path.join('styles', 'css'),
			images: path.join('assets'),
		...
		for (var key in folders) {
			...
			options.folders.dist[key] = path.join(
				options.root,
				options.outputFolder,
				folders[key]
			); /* Destination folder of modules,
			can be changed if needed. */
		}
	},

	// 'development' is the default flag
	// --watch flag comes under 'development'
	...
	build: async function (options, dependencies) {
		...
		if (options.transpile) {
			options.ignoreFoldersFromTranspile = ['node_modules'];
		}
		...
		/* Copy env.js files */
		buildUtils.copy.file({
			src: path.join(buildOptions.root, `/env/${options.env}/env.conf.js`),
			dist: path.join(buildOptions.outputFolder, '/env.js'),
			minify: true
		});

		// Copy Monaco Themes
		buildUtils.copy.folder({
			src: path.join(buildOptions.root, '/node_modules/monaco-themes/themes'),
			dist: path.join(buildOptions.outputFolder, `/editor/themes`)
		});
		...
		/* Ignore dev folders in output folder */
		const ignoreFolders = ['components', 'data-store', 'mixins', 'node_modules', 'routes', 'scripts', 'styles'];
		ignoreFolders.forEach((folder) => {
			buildUtils.delete(path.join(buildOptions.outputFolder, folder));
		});
	},
	/*-----------------------------------------------Build Process ends---------------------------------------------*/
	...
	builder: {
		...
		copyAppDir: async function (module) {
			...
			const ignoreFolders = ['dist', 'env', 'node', 'release', 'src', 'target'];
		...
		components: async function (module) {
			includeStyle = false; /* styles will be included to template by default. If not needed, toggle this property. */
